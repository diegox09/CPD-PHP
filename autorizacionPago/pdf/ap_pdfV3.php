<?php
	...
	if($logonSuccess){		
		...
		foreach ($array as $id) {
			...
			if($fecha_ap == "" || (strtotime($fecha_ap) >= strtotime($fecha_v3))){
		...
		if($fecha == "" || (strtotime($fecha_ap) >= strtotime($fecha_v3)))
			$pdf->Output('MMPA-02-F-01-3 Autorización de Pago V3.pdf', 'I');
		else
			$pdf->Output('MMPA-02-F-01-3 Autorización de Pago V2.pdf', 'I');
