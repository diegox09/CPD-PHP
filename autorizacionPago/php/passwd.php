<?php
	...
	$result = AutorizacionPago::getInstance()->get_passwd_by_user($usuario);
	...
	if(mysqli_num_rows($result) == 0)	
		...
		while ($user = mysqli_fetch_array($result)){
			$consulta = true;				
			$nombre = utf8_encode($user['nombre']);
			$passwd = utf8_encode($user['passwd']);
			$email = utf8_encode($user['emailUser']);				
			
			$body = '<p>Cordial Saludo,</p>
					<p><strong>'.$nombre.'</strong></p>
					<div style="text-align:justify">Datos de ingreso</div>
					<p>Usuario: <strong>'.$usuario.'</strong></p> 
					<p>Contraseña: <strong>'.$passwd.'</strong></p>
					<p><a href="sgc.corprodinco.org/autorizacionPago" >sgc.corprodinco.org/autorizacionPago</a></p>
