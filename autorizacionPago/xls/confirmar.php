    ...
    <div id="info" style="background:#f5f5f5">    	
    	<div class="info">
			<?php
                    ...
                    if(move_uploaded_file($_FILES['userfile']['tmp_name'], $temp)){
                        if(file_exists ($temp) && is_readable ($temp)){																		
                            ...
                            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                                ...
                                if($worksheet->getCellByColumnAndRow(1, 1)->getValue() == 'PROGRAMA' && $worksheet->getCellByColumnAndRow(3, 1)->getValue() == 'MUNICIPIO' && $worksheet->getCellByColumnAndRow(5, 1)->getValue() == 'FECHA' && $worksheet->getCellByColumnAndRow(6, 1)->getValue() == 'NIT/C.C.' && $worksheet->getCellByColumnAndRow(10, 1)->getValue() == 'CUENTA'){ 
                                    ...
                                    for ($row = 2; $row <= $highestRow; ++ $row) {											
                                        ...
                                        $iva = trim($worksheet->getCellByColumnAndRow(13, $row)->getValue()) * 100;										
                                        $valorIva = strtr(trim($worksheet->getCellByColumnAndRow(14, $row)->getValue()), $quitar);
										$retencionIva = trim($worksheet->getCellByColumnAndRow(15, $row)->getValue()) * 100;										
                                        $valorRetencionIva = strtr(trim($worksheet->getCellByColumnAndRow(16, $row)->getValue()), $quitar);
										$retencionFuente = trim($worksheet->getCellByColumnAndRow(17, $row)->getValue()) * 100;										
										...
										if($idPrograma != '' && $idResponsable != '' && $idMunicipio != '' && $idCliente != '' && $concepto != ''){
											...
											if(mysqli_num_rows($verificar) != 0){
												...
												$errores[] = $row;
												echo "<div class='error'>Autorizacion de Pago No Existe - Fila: ".$row."</div>";
											}
										}
				...
				echo '<input type="hidden" id="lista" value="'.$imprimir.'"/>';				          
            ?>
            <div id="botones">
             	<input type="button" id="regresar" class="boton_gris" value="Regresar" title="Regresar"/>
                &nbsp;&there4;&nbsp;
                <input type="button" id="deshacer" class="boton_naranja" value="Deshacer" title="Deshacer"/>
                &nbsp;&there4;&nbsp;   
                <input type="button" id="imprimir" class="boton_azul" value="Imprimir" title="Imprimir"/>           
          	</div>      
    	</div>
    </div> 
