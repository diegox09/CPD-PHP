<?php
	...
	if($logonSuccess){
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('I17', 'No. Arriendos entregados')
					->setCellValue('J16', 'Valor de los Arriendos Entregados')
		...
		while ($info = mysqli_fetch_array($result)){
			...
			$valorArriendo = $contArriendo * 700000;	
			...
			$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue("G".$fila_2, "1")			
						->setCellValue("I".$fila_2, $contArriendo)	
						->setCellValue("K".$fila_2, $valorArriendo);			
		...
		if($fila_2 >= 18){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$contador, '=SUM(K18:K'.$fila_2.')');
		}
		...
		$objPHPExcel->getActiveSheet()->getStyle('K'.$contador)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		...
		$objPHPExcel->getActiveSheet()->getStyle('K18:K'.$contador)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
		...
		header('Content-Disposition: attachment;filename="CF-CH-002-Alojamientos_Temporales.xls"');
