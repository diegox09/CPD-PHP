<?php
	...
	if($logonSuccess){
		...
		$objPHPExcel->setActiveSheetIndex(0)			
					...
					->setCellValue('F11', 'Entidad Interventora:  COOPROCONAS')
		...
		//negrita
		$objPHPExcel->getActiveSheet()->getStyle('A1:S17')->getFont()->setBold(true);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A1:S15')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('J16', 'KIT DE MERCADOS ENTREGADOS')
					->setCellValue('J17', 'Tipo A De 0 a 3 integrantes')
					->setCellValue('K17', 'Tipo B De 3 a 6 Integrantes')
					->setCellValue('L17', 'Tipo C Mas de 6 Integrantes')
					->setCellValue('M17', 'Valor de las Ayudas Entregadas')
					->setCellValue('N16', 'KIT DE ASEO ENTREGADOS')
					->setCellValue('N17', 'Tipo A De 0 a 3 integrantes')
					->setCellValue('O17', 'Tipo B De 3 a 6 Integrantes')
					->setCellValue('P17', 'Tipo C Mas de 6 Integrantes')
					->setCellValue('Q17', 'Valor de las Ayudas Entregadas')
					->setCellValue('R16', 'Arriendos Entregados')	
					->setCellValue('S16', 'Valor de los Arriendos Entregados')
					->setCellValue('S17', 'Valor de las Ayudas Entregadas');	
		...
		$objPHPExcel->getActiveSheet()->mergeCells('N16:Q16');
		$objPHPExcel->getActiveSheet()->mergeCells('R16:R17');
		
		//alinear
		$objPHPExcel->getActiveSheet()->getStyle('A16:S17')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('A16:S17')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		...
		$totalArriendos = mysqli_num_rows($result);		
		while ($info = mysqli_fetch_array($result)){
		...
		$objPHPExcel->getActiveSheet()->getStyle('E'.$fila.':S'.$fila)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFC0C0C0');					
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("I".$fila_2, $totalArriendos)
					->setCellValue("K".$fila_2, $contMercados)
					->setCellValue("M".$fila_2, $valorMercados)
					->setCellValue("O".$fila_2, $contKitsAseo)				
					->setCellValue("Q".$fila_2, $valorKitsAseo)
					->setCellValue("R".$fila_2, $contArriendos)
					->setCellValue("S".$fila_2, $valorArriendos);
			
		$objPHPExcel->getActiveSheet()->getStyle('A16:S'.$fila_2)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		...
		$objPHPExcel->getActiveSheet()->getStyle('I'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED2);		
		$objPHPExcel->getActiveSheet()->getStyle('K'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED2);
		$objPHPExcel->getActiveSheet()->getStyle('O'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED2);
		$objPHPExcel->getActiveSheet()->getStyle('R'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED2);
		
		$objPHPExcel->getActiveSheet()->getStyle('M'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
		$objPHPExcel->getActiveSheet()->getStyle('Q'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
		$objPHPExcel->getActiveSheet()->getStyle('S'.$fila_2)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
			
		$objPHPExcel->getActiveSheet()->getStyle('A1:S'.$fila_2)->getFont()->setSize(10);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A16:S17')->getAlignment()->setWrapText(true);
