...
(function($){	
    $.fn.limpiarBeneficiario=function(){	
		...
		$('#id_beneficiario_psicologia').cargarInput('');
...
(function($){	
    $.fn.cargarBeneficiario=function(respuesta){
		...
		$('#id_beneficiario_psicologia').cargarInput(respuesta.idBeneficiario);
...
(function($){	
    $.fn.cargarPronino=function(respuesta){
		...
		$('#razon_egresado').cargarSelect(respuesta.razonEgresado);
		$('#razon_baja').cargarSelect(respuesta.razonBaja);
...
(function($){	
    $.fn.limpiarYear=function(){	
		...
		$('#kit_nutricional').cargarInput('');
		$('#uniforme').cargarInput('');
		$('#zapatos').cargarInput('');	
		$('#visita_domiciliaria').cargarInput('');	
		$('#visita_seguimiento').cargarInput('');	
...
(function($){	
    $.fn.cargarYear=function(respuesta){
		...
		$('#kit_nutricional').cargarInput(respuesta.kitNutricional);
		...
		$('#visita_seguimiento').cargarInput(respuesta.visitaSeguimiento);
...
(function($){	
    $.fn.contarActividades=function(){	
		...
		for(var i=1; i<7; i++){						
			if(actividades[i] != 0)	
				actividades[i] = actividades[i]+' Hora(s)';
			else
				actividades[i] = '';				
    ...
    $.fn.cargarMes=function(respuesta){		
		...
		for(var i=0; i<6; i++){
			if(respuesta.periodoLectivo[i] != 0)	
				respuesta.periodoLectivo[i] = respuesta.periodoLectivo[i]+' Hora(s)';
			else
				respuesta.periodoLectivo[i] = '';
			$('#pl_actividad_'+i).html(respuesta.periodoLectivo[i]);
			if(respuesta.periodoVacaciones[i] != 0)
				respuesta.periodoVacaciones[i] = respuesta.periodoVacaciones[i]+' Hora(s)';
			else
		...
		for(var i=0; i<6; i++){
			if(respuesta.meses[i] != 0)
				respuesta.meses[i] = respuesta.meses[i]+' Hora(s)';
			else
				respuesta.meses[i] = '';	
		...
		for(var i=0; i<4; i++){
			if(respuesta.horas[i] != 0)
				respuesta.horas[i] = respuesta.horas[i]+' Hora(s)';
			else
				respuesta.horas[i] = '';
		...
		for(var i=0; i<3; i++){
			if(respuesta.dias[i] != 0)
				respuesta.dias[i] = respuesta.dias[i]+' Hora(s)';
			else
				respuesta.dias[i] = '';
...
(function($){	
    $.fn.cargarListaPsicosocial=function(respuesta){
		...
		$('#tabla_psicosocial').append('<thead><tr><th width="15%">Fecha</th><th width="60%">Aspecto donde presenta Dificultad</th><th width="25%">Profesional</th></tr></thead><tbody id="desc_psicosocial"></tbody>');
                
		if(respuesta.lista){
			var aspecto = "";					
			for(i=0; i<respuesta.idPsicosocial.length; i++){
				aspecto = "";
				if(respuesta.aspectoAcademico[i] == 1)
					aspecto = "Academico";
				if(respuesta.aspectoComportamiento[i] == 1){
					if(aspecto != "")
						aspecto = aspecto + ", Comportamiento";
					else
						aspecto = "Comportamiento";
				}
				if(respuesta.aspectoComunicativo[i] == 1){
					if(aspecto != "")
						aspecto = aspecto + ", Comunicativo";
					else
						aspecto = "Comunicativo";
				}				
				if(respuesta.aspectoFamiliar[i] == 1){
					if(aspecto != "")
						aspecto = aspecto + ", Familiar";
					else
						aspecto = "Familiar";
				}
				$('#desc_psicosocial').append('<tr class="ver_psicosocial" id="'+respuesta.idPsicosocial[i]+'" title="'+respuesta.nombreUser[i]+' &raquo; '+respuesta.fechaActualizacion[i]+'"><td>'+respuesta.fechaRemision[i]+'</td><td>'+aspecto+'</td><td>'+respuesta.nombreProfesional[i]+'</td></tr>');	
			}			
		}
...
(function($){	
    $.fn.limpiarPsicologia=function(){	
		$('#form_psicologia').find('.input_error').removeClass('input_error');
		$('#usuario_actualizo_psicologia').html('-');
		$('#fecha_actualizacion_psicologia').html('-');	
		$('#id_psicologia').cargarInput('');		
		$('#nombre_beneficiario_psicologia').cargarInput($('#nombres').attr('value')+' '+$('#apellidos').attr('value'));	
		
		$('#fecha_atencion_psicologia').cargarInput('');			
		$('#observaciones_psicologia').cargarTextarea('');
		$('#impresion_diagnostica').cargarTextarea('');
		$('#plan_intervencion').cargarTextarea('');		
					
		if($('#id_estado').attr('value') == 2){
			$('#form_psicologia input[type="text"], #form_psicologia textarea, #form_psicologia select').attr('readonly', true);
			$("#guardar_psicologia").attr("disabled","disabled");
		}
		else{
			$('#form_psicologia input[type="text"], #form_psicologia textarea, #form_psicologia select').not('#nombre_beneficiario_psicologia').removeAttr('readonly');
			$("#guardar_psicologia").removeAttr("disabled");
		}
		
		$("#eliminar_psicologia").hide();
	}
})(jQuery);

(function($){	
    $.fn.cargarPsicologia=function(respuesta){
		$('#form_psicologia').find('.input_error').removeClass('input_error');
		$('#usuario_actualizo_psicologia').html(respuesta.nombreUser[0]);
		$('#fecha_actualizacion_psicologia').html(respuesta.fechaActualizacion[0]);	
		$('#id_psicologia').cargarInput(respuesta.idSeguimiento);		
		$('#nombre_beneficiario_psicologia').cargarInput($('#nombres').attr('value')+' '+$('#apellidos').attr('value'));	
		
		$('#fecha_atencion_psicologia').cargarInput(respuesta.fechaAtencionPsicologia);	
		$('#observaciones_psicologia').cargarTextarea(respuesta.observacionesPsicologia);
		$('#impresion_diagnostica').cargarTextarea(respuesta.impresionDiagnostica);
		$('#plan_intervencion').cargarTextarea(respuesta.planIntervencion);
		...
		if(respuesta.perfil > 0)	
			$("#eliminar_psicologia").show();
		else
			$("#eliminar_psicologia").hide();	
			
		$('#actualizar_psicologia').focus();
	}
})(jQuery);

(function($){	
    $.fn.cargarListaPsicologia=function(respuesta){
		$.fn.limpiarPsicologia();		
		$('#tabla_psicologia').empty(); 
		
		$('#tabla_psicosologia').append('<thead><tr><th width="15%">Fecha</th><th width="85%">Observaciones</th></tr></thead><tbody id="desc_psicologia"></tbody>');
                
		if(respuesta.lista){
			var aspecto = "";					
			for(i=0; i<respuesta.idPsicologia.length; i++){				
				$('#desc_psicologia').append('<tr class="ver_psicologia" id="'+respuesta.idPsicologia[i]+'" title="'+respuesta.nombreUser[i]+' &raquo; '+respuesta.fechaActualizacion[i]+'"><td>'+respuesta.fechaAtencionPsicologia[i]+'</td><td>'+respuesta.observacionesPsicologia[i]+'</td></tr>');	
			}			
		}	
		
		$('#tabla_psicologia').tablesorter({widgets: ['zebra']});
		$('#actualizar_psicologia').focus();
	}
})(jQuery);

(function($){	
    $.fn.psicologia=function(opc){		
		if((opc == 'cargar' && $('#id_psicologia').obligatorio()) || (opc == 'cargar_lista' && $('#id_beneficiario_psicologia').obligatorio()) || (opc == 'eliminar' && $('#id_psicologia').obligatorio()) || ($('#id_beneficiario_psicologia').obligatorio() && $('#fecha_atencion_psicologia').obligatorio())){			
			$("#guardar_psicologia").attr("disabled","disabled");			
			$('#cargando').show();
			$('#opc_psicologia').cargarInput(opc);													
			var datos = $('#form_psicologia').serialize();			
			$.getJSON('php/beneficiario_psicologia.php', datos, function(respuesta) {
				if(respuesta.login){					
					if(respuesta.consulta){	
						switch(respuesta.opc){
							case 'cargar': 	$.fn.cargarPsicologia(respuesta);
											$('#error').html('Seguimiento de Atencion Psicologica cargado correctamente');
											break;
							case 'cargar_lista':$.fn.cargarListaPsicologia(respuesta);
												$('#error').html('Registro de Seguimientos de Atencion Psicologica');
												break;				
							case 'eliminar':$.fn.cargarListaPsicologia(respuesta);
											$('#error').html('Seguimiento de Atencion Psicologica eliminado correctamente');
											break;				
							case 'guardar':	$.fn.cargarListaPsicologia(respuesta);
											$('#error').html('Seguimiento de Atencion Psicologica guardado correctamente');
											break;							
						}								
					}
					else{
						switch(respuesta.opc){
							case 'cargar':	$.fn.limpiarPsicologia();
											$('#error').html('Error al cargar el Seguimiento de Atencion Psicologica');
											break;
							case 'cargar_lista':$.fn.cargarListaPsicologia(respuesta);
												$('#error').html('Error al cargar los Seguimientos de Atencion Psicologica');
												break;				
							case 'eliminar':$.fn.cargarListaPsicologia(respuesta);
											$('#error').html('Error al eliminar el Seguimiento de Atencion Psicologica');																					
											break;				
							case 'guardar':	$.fn.cargarListaPsicologia(respuesta);
											$('#error').html('Error al guardar el Seguimiento de Atencion Psicologica');
											break;						
						}
					}					
				}
			...
			.error(function() { 
				$("#guardar_psicologia").removeAttr("disabled");
				$("#cargando").delay(400).slideUp(0);
				$('#error').html("Error: Compruebe la conexion de red de su equipo! - psicologia"); 
			});	
		}
    }
})(jQuery);
...
$(document).ready(function(){
	...
	*/	
	...
	$("#kit_nutricional").datepicker();
	$("#uniforme").datepicker();
	$("#zapatos").datepicker();	
	$("#visita_domiciliaria").datepicker();
	$("#visita_seguimiento").datepicker();
	...
	//Atencion Psicologia
	$("#ver_psicologia").click(function(event){
		$.fn.psicologia('cargar_lista');					
		$('#overlay').show();
		$('#psicologia').show();		
	});	
	
	$("#actualizar_psicologia").click(function(event){
		$.fn.psicologia('cargar_lista');		
	});		
	
	$("#form_psicologia").submit(function(event){
		event.preventDefault();
		$.fn.psicologia('guardar');
	});
	
	$("#imprimir_psicologia").click(function(event){
		window.open('pdf/atencion_psicologia.php?id_beneficiario='+$('#id_beneficiario_psicologia').attr('value'));
	});
	
	$(".ver_psicologia").live('click', function(event){
		$('#id_psicologia').cargarInput($(this).attr('id'));
		$.fn.psicologia('cargar');			
	});
	
	$("#eliminar_psicologia").click(function(event){
		if(confirm('Desea eliminar la atencion psicologica'))	
			$.fn.psicologia('eliminar');
	});
	
	$("#cancelar_psicologia").click(function(event){
		$.fn.limpiarPsicologia();	
		$('#overlay').hide();			
		$('#psicologia').hide();	
	});
	...
	$('#situacion_laboral').textareaCount(options_sl);
	...
	$('#descripcion_escenarios').textareaCount(options_de);
	...
	$('#observaciones_diagnostico').textareaCount(options_od);
	...
	$('#motivo_seguimiento').textareaCount(options_ms);
	...
	$('#descripcion_seguimiento').textareaCount(options_ds);
	...
	$('#motivo_aspecto_academico').textareaCount(options_maa);
	...
	$('#motivo_aspecto_comportamiento').textareaCount(options_mac);
	...
	$('#motivo_aspecto_comunicativo').textareaCount(options_mac2);
	...
	$('#motivo_aspecto_familiar').textareaCount(options_maf);
	...
	$('#acciones_realizadas').textareaCount(options_ar);
