	...
	if($logonSuccess){
		...
		while ($id = mysqli_fetch_array($result)){
			...
			for($i=0; $i<6; $i++){
				...
				$trabajoInfantil += $mesesTI[$i];
			}
			
			if($trabajoInfantil == 0)
				$trabajoInfantil = "";
			else	
				$trabajoInfantil = "X";
			...
			$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue('N'.$fila, $trabajoInfantil)
						->setCellValue('O'.$fila, $mesesTI[0])
						->setCellValue('P'.$fila, $mesesTI[1])
						->setCellValue('Q'.$fila, $mesesTI[2])
						->setCellValue('R'.$fila, $mesesTI[3])
						->setCellValue('S'.$fila, $mesesTI[4])
						->setCellValue('T'.$fila, $mesesTI[5])
