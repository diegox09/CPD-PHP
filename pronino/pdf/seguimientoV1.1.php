<?php
	...
	if($logonSuccess){	
		...
		if(mysqli_num_rows($result) == 0){
			...
			$pdf->Image('../img/logo.png',210,18,41);		
			...
			$observaciones = 'DESCRIPCION DE LOS ESCENARIOS DE PARTICIPACIÓN DEL BENEFICIARIO dddddddddd OBSERVACIONES, RECOMENDACIONES Y/O SUGERENCIAS (A nivel familiar, social, escolar, comportamental y académico)';				
		...
		while ($beneficiario = mysqli_fetch_array($result)){
			...
			if(($i % 4) == 0){	
				...
				$pdf->SetXY(200, 15);
				$pdf->Cell(65,20,'',1,1,'C');
				$pdf->Image('../img/logo.png',215,16,31);		
